...
const SelectGrammarLesson = ({ navigation, route }) => {
  ...
  const { getGrammarLessons, isPreloading } = useFreeDocument();

  useEffect(() => {
    // Lấy data từ context thay vì gọi API
    const lessons = getGrammarLessons(level);
    setAllLessons(lessons);
  }, [level, getGrammarLessons]);

  const handleLessonPress = (lesson) => {
    navigation.navigate("GrammarLessonDetail", { lesson });
  };
    ...
    >
      <View className="flex-row items-center">
        ...
        <View className="flex-1">
          ...
          <View className="flex-row items-center">
            <Icon name="library-outline" size={14} color="#6B7280" />
            <Text className="text-sm text-gray-500 ml-1">
              {item.grammarCount} ngữ pháp
            </Text>
          </View>
        </View>
